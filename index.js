(function (root, factory) {
  ...
  DrawingManager.prototype._bindPolylineOrPolygon = function () {
    ...
    var startAction = function (e) {
      ...
      if (!isBinded) {
        ...
        mask.addEventListener('rightclick', rightclickAction);
      }
    };
     ...
     * 鼠标右击的事件
     */
    var rightclickAction = function (e) {
      baidu.stopBubble(e);
      isBinded = false;
      mask.disableEdgeMove();
      mask.removeEventListener('mousemove', mousemoveAction);
      mask.removeEventListener('rightclick', dblclickAction);
      overlay.setPath(points);
  ...
  DrawingManager.prototype._dispatchOverlayComplete = function (overlay, calculate) {
    ...
    //如果绘制的面积等于0，则删除该overlay，不派发事件
    var overlayType = overlay.toString();
    if((overlayType.indexOf("Polygon") > -1 || overlayType.indexOf("Circle") > -1) && (calculate.data || 0) ===0){
      this._map.removeOverlay(overlay);
      return;
    }
    if (calculate) {
    ...
    Mask.prototype._bind = function () {
      ...
      var domEvent = function (e) {
        ...
        if (type == "mousedown") {
          ...
          //鼠标右键
          if (e.buttons === 2) {
            var ev = new MouseEvent('rightclick', {
              cancelable: true,
              bubble: true,
              view: window
            });
